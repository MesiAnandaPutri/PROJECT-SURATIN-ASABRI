<x-app-layout>
    <div class="mb-6 flex justify-between items-center">
        <div>
            ...
            <p class="text-gray-600">Manajemen surat dan berkas keluar.</p>
        </div>
    ...
    <div class="bg-white rounded-lg shadow-sm p-4 mb-6">
        <form action="{{ route('surat-keluar.index') }}" method="GET" class="flex gap-4">
            <div class="flex-1">
            ...
            <table class="min-w-full divide-y divide-gray-200">
                <thead class="bg-asabri-blue text-white">
                    <tr>
                        ...
                        <th class="px-6 py-4 text-left text-xs font-bold uppercase tracking-wider">Tujuan</th>
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    @forelse($suratKeluar as $index => $surat)
                        <tr class="hover:bg-gray-50 transition-colors">
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                {{ $index + $suratKeluar->firstItem() }}</td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                {{ \Carbon\Carbon::parse($surat->tanggal_surat)->format('d M Y') }}</td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900 font-medium">{{ $surat->tujuan }}
                            </td>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full 
                                    {{ $surat->status === 'Draft' ? 'bg-gray-100 text-gray-800' : '' }}
                                    {{ $surat->status === 'Dikirim' ? 'bg-blue-100 text-blue-800' : '' }}
                                    {{ $surat->status === 'Disetujui' ? 'bg-green-100 text-green-800' : '' }}">
                                    {{ $surat->status }}
                                </span>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium flex gap-2">
                                <!-- Detail Button (Trigger Modal in future) -->
                                ...
                                @if(in_array(Auth::user()->role, ['admin', 'staff']))
                                    <!-- Hapus -->
                                    <form action="{{ route('surat-keluar.destroy', $surat->id) }}" method="POST" class="inline" onsubmit="return confirm('Apakah Anda yakin ingin menghapus surat ini?')">
                                        @csrf
                                        @method('DELETE')
                    ...
                    @empty
                        <tr>
                            <td colspan="7" class="px-6 py-4 text-center text-gray-500">Tidak ada data surat keluar.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
